...
The planning problem is defined as the **root operator** <+root> which methods are potential solutions of the problem. Its preconditions and effects are respectively used as initial state and goal description.
::::::::::::::::::::::::::::::::::::
...
Since we base our planning formalism on the one of <+self>, we can express the structure of our formalism using the same extended Venn diagram as in @fig:typerel. The @fig:color explains how planning uses the previously defined relations to make its structure homogeneous and contained within the universe <+uv>. We can also see the relation $\landor$ between the set of all states <+states> and the set of all fluents <+flu> that forms the previously shown and-or trees. Multiple relations are used for actions in <+actions>. Those are all explained in @def:action. We included the root operator <+root> in the set of all actions. It is interesting to note how the set of plans <+plans> is by definition a set of graphs since it uses a connectivity function <+con> between actions and states.
...
In diverse planning, other criteria that are user specified is the cardinality of the set of solutions. We note <+nsol> the number of expected different solutions. This simply makes the process return when either it found <+nsol> solutions or when it determined that $\nsol > |\solu|$.
...
$$\plans^*_{\states} = \plans^* \left( (\states, \actions), \pre(\root), \eff(\root) \right)$$
...
with $a^0$ and $a^*$ being the initial and goal steps of the plan corresponding to $\start$ such that $\eff(a^0) = \pre(\root)$ and $\pre(a^*) = \eff(\root)$. The iterator is all the possible resolutions of all flaws on any plan in the search space and the solution predicate is true when the plan has no more flaws.
...
$$\plans_{\cal{C}} = \plans^* \left( r_* , \lBrace \min : \plan \in \cal{C} \land \plan(\pre(\root)) \models \eff(\root) \rBrace, \search(\pre(\root)) \neq \emptyset \right)$$
...
$$\plans_{\proba} = \plans^* \left( \bb{pol}, \pre(\root), \search \models \eff(\root) \right )$$
...
$$\plans_{\root} = \plans^* \left ( r_+, \plans({\root}), \otimes(\search) = \emptyset \land \univ a \in \actions_{\plan \in \search} \plans(a) = \emptyset \right )$$
