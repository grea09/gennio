...
```{.syntax #lst:grammar caption="Simplified pseudo-<+bnf> description for basic <+self>." escapechar=$}
<~COMMENT: <INLINE: "//" (~["\n", "\r"])*>
| <BLOCK: "/*" (~["*/"])*> > //Ignored
<~WHITE_SPACE: " "|"\t"|"\n"|"\r"|"\f">
...
```{.syntax #lst:container caption="Rules added to the current grammar for handling the new container for parenthesis" escapechar=$}
...
```{.syntax #lst:parameter caption="Rules added to the current grammar for handling parameters" escapechar=$}
...
```{.syntax #lst:modifier caption="Rules added to the current grammar for handling modifiers" escapechar=$}
<meta_entity> ::= <?MOD> <ID>
...
```{#lst:lang caption="The default lang.w file." escapechar=$}
* =? ;$\label{line:first}$
...
``` {.rdf language=rdf #lst:modal_rdf caption="RDF representation of the gossip example"}
...
``` {#lst:modal_self caption="<+self> representation of the gossip example"}
"lang.s" = ? ;
