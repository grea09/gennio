...
The *combination function* is the function that associates any two function $f_1$ and $f_2$ to a new function that associates any functions associated by **either** functions. [^Formally $f_1 \bowtie f_2 = x \to y$ with $f_1(x) = y$ **or** $f_2(x) = y$.]
:::
...
^[Formally $f_1 \vartriangle f_2 = x \to y$ with $f_1(x) = f_2(x) = y$.]
:::

::: {.definition #def:complement name="Complement"}
The *complement function* is the function that associates any functions to a new function that associates any symbols **not** associated by the function.
^[Formally $\triangledown f = x \to y$ with $y \triangledown(=) x$.]
...
Now we can also define the other couple of functions on functions.
...
From now on we will use numbers and classical arithmetics on them. From a fondation point of view, these are defined later using Peano's axiom.
...
In this section, we present First Order Logic (FOL). All notations are presented in @tbl:logic. FOL is based on boolean logic with the two constants $\top$ *true* and $\bot$ *false*. 

A function that have as only values either $\top$ or $\bot$ is called a **predicate**.

We define the classical logic *entailment* as $\vdash$
...
functions that takes an expression as parameter are called *modifiers*. FOL introduce a useful kind of modifer used to generalize expressions: *quantifiers*. Quantifiers takes an expression and a variable as arguments. Classical quantifiers are also predicates: they restrict the values that the variable can take.
...
The member function is noted $e \in \cal{S}$ to indicate that $e$ is an element of $\cal{S}$.
...
We can combine functions using the *function composition operator* $g \circ f = x \to g(f(x))$. The functional power of $f$ noted $f^n$ is defined recursively as :

* $f^0 = x \to x$, $f^1 = x \to f(x)$ and $f^{-1} = f(x) \to x$
