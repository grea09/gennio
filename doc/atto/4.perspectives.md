...
form used by @avrahami-zilberbrand_fast_2005b. The tree consists of nodes that
...
Another contextual approach is the one developed for robotics by @hofmann_intent_2007a. The stochastic system is completed by a weighting based on
...
problems. This method was used by @chen_planning_2013 (based on the work of
@ramirez_probabilistic_2010) to solve the problem with very mature tools. The
model consists in defining a classical planning problem and modifying it
by introducing observations and their sequentiality constraints in the
form of artificial predicates of the execution of observed actions. By
using the difference in weights between the optimal designs taking into
account observations and not by effectively determining the most likely
designs.
