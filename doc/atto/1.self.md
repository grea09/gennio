...
This theorem comes from the @axi:extensionality of ZFC. Indeed it is stated that a set is a unordered collection of distinct objects. Distinction is possible if and only if intrinsic identity is assumed.
...
The uniformity of SELF also allows some very interesting meta-constructs. That is why we also introduce a **meta relation** $\mu^\pm: E \rightleftharpoons^+_- D$. This allows to create domain from certain entities and to encapsulate domains into entities. $\mu^-$ is for reification and $\mu^+$ is for abstraction.
...
```{#lst:grammar .java caption="Simplified pseudo-BNF description for basic SELF." escapechar=$}
<~COMMENT: <INLINE: "//" (~["\n", "\r"])*>
|	<BLOCK: "/*" (~["*/"])*> > //Ignored
<~WHITE_SPACE: " "|"\t"|"\n"|"\r"|"\f">
