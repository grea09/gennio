...
Abstraction will associate a *definition* as a formula describing a term. And here, another problem arise.
...
Defining a term requires using a formula in the host language to express the abstracted properties of the generalization. The problem is that most terms will have *circular* definitions. 
...
#### Construction toward arithmetic
...
Since most of this knowledge representation is effectively made by humans and group of people, this representation is heavily influenced by context and culture. The problem with trying to encode knowledge for artificial systems is that they lack the cultural and societal bagage that is learnt by people throughout their lives. This leads to very human centric way of expressing knowledge in computer science. This can be seen as well in the way knowledge is comunicated between researchers. Groups form based on domain expertise. These groups are unintentionally not very permeable. Nowadays, specialization is prefered to generalization and cross domain works requires more and more efforts to organize. This leads to the creation and derivation of languages to express sometimes very similar ideas. For example, in mathematics, each subdomain will have a proper jargon to talk about its notions. This means that the fondation of mathematics (and therefore computer science) is based on a very human centric understanding of the subject. This makes mathematics more of a loosly bound network of dialects that reflects often the same notions through different names. Some of the bases of the knowledge is also transmited as folklore between teachers and students without some precisely redacted document explaining the notions.

**TODO** Citations

This is why, when investigating the way to formally define a base language with as little asumptions as possible, it becomes clear that the bases lay on imprecise formalism. The most accepted version of fondation of mathematics is the Zermelo-Fraenkel axiomatic set theory with the axiom of Choice (ZFC) [@fraenkel_foundations_1973; @ciesielski_set_1997]. This theory is the ground work for most of the classical mathematics still used to this day. However, several assumptions are made in the theory that are not explicitely stated. For example, when naming an element or a set we supose the principle of identity as we can distinguish them from one another. Also, the notion of set isn't properly defined as well as the classical quantifiers that are used in even the very first formula of the theory.

When trying to define the very base of such a precise knowledge system, it is clear that some terms have cyclical dependencies. Also, it is impossible to formally define a formalism without using a pre-existing formalism. Indeed, a *formalism* is a language along with a set of rules that allows to express *formulas* using terms and operations. This is why the base of any axiomatic theory can't be formally defined. By aknowledging that fact, we can start with the smalest possible definitions that allows the earliest formalisation of later fundamental terms. In this chapter, we expose a potential solution that clearly states the parts that are undefinable along side a computer knowledge representation system based on this mathematical theory.


## A functional theory of mathematics

Before explaining our solution, we need to define the bases of the whole system. This base is a possible fondation of mathematics that aims to reduce the number of axioms and assumed terms and operation.
...
Mathematics needs a fondation. Several theories have been proposed as the fondation of mathematics (set theory, group theory, model theory, etc). In this part, as an introduction to the fundamentals of maths and logic, we propose another view of that fondation based on functions. The unique advantage of it lays in its explicit structure that have emergent reflexive properties. It also holds a coherent algebra that have a strong expressive power.

Our theory is axiomatic, meaning it is based on fundamental logical proposition called axioms. Those are supposed to be true without any need for proof. The following axioms are the mandatory undefinable notions created from natural language. Those are explicit bases of the formalism.

::: {.axiom #axi:symbolism name="Symbolism"}
Lets be symbols.
:::

::: {.axiom #axi:function name="Function"}
Lets be functions between symbols.
:::

::: {.axiom #axi:reflexivity name="Reflexivity"}
Functions are symbols.
:::

While simple these axioms are very important. Indeed, it is not possible to formally them define yet because we are defining the formalism. This means that we need to use functions and symbols to express their definition. Functions are often noted using the prefix notation delimited by parenthesis $f(args)$ allows to get the image of arguments associated by the function $f$. If a function only associate two symbols, we can also use the infix notation e.g. $x f y$.

::: {.definition #def:null name="Null"}
The *null function* is the function between nothing and nothing. We note it $\gtrdot$.
:::

::: {.definition #def:identity name="Identity"}
The *identity function* is the function that associates any symbol to itself noted $=$.
:::

---

An **expression** is any coherent combination of **terms**. Each term must be either a terminal symbol or a relation properly applied to arguments. This can be recursive as relations can take the value of another relation as argument.

A relation can be referenced to by its symbol or defined anonymously using the *mapping relation* $x \mapsto f(x)$ with $f(x)$ being a valid expression depending on $x$.
...
From now on we will use numbers and classical arithmetics on them. From a fondation point of view, these are defined later using @axi:peano.
