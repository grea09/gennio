...
$\bowtie, \vartriangle, \triangledown$                Combination, superposition and complent relations.
$\circ, \bullet$                                      Composition and inverse relations.
$|r|$                                                 Arity of a relation.

: List of classical symbols for relations. {#tbl:relation}

Mathematics needs a fondation. Several theories have been proposed as the fondation of mathematics (set theory, group theory, model theory, etc). In this part, as an introduction to the fundamentals of maths and logic, we propose another view of that fondation based on relations. The unique advantage of it lays in its explicit structure that have emergent reflexive properties. It also holds a coherent algebra that have a strong expressive power.

::: {.axiom #axi:symbolism name="Symbolism"}
Let be symbols.
:::

::: {.axiom #axi:relation name="Relation"}
...
While simple these axioms are very important. Indeed, it is not possible to formally define yet because we are defining the formalism. This means that we need to use relations and symbols to express their definition. That isn't a big problem in itself since most of the human vocabulary, for example, is circular. All languages have cyclical definitions for basic terms.

The prefix notation delimited by parenthesis $r(args)$ allows to get the image of arguments associated by the relation $r$. If a relation only associate one symbol to one other symbol, we can also use the infix notation e.g. $arg_1 r arg_2$.

::: {.definition #def:null name="Null"}
The *null relation* is the relation between nothing and nothing. We note it $\gtrdot$.
:::

::: {.definition #def:identity name="Identity"}
The *identity relation* is the relation that associates any symbol to itself noted $=$.
:::
