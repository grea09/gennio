...
$^{\rightdowncurvedarrow}_{\rightupcurvedarrow}\bullet$
...
We usually note a graph $g=(V,E)$ with the set of vertices $V$ (also called nodes) and edges $E$ (arcs) that links two vertices together. Each edge is basically a pair of vertices ordered or not depending on if the graph is directed or not.^[$E \subseteq V^2$]
