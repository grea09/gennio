...
* *Adjacency*: $\chi_{\adja} = v \asso \{ e: v \in \chi_{\inci}(e) \}$^[Also: $\chi_{\adja} = \inv \chi_{\inci}$]
* *Incidence*: $\chi_{\inci} = e \asso \{ v: e \in \chi_{\adja}(v) \}$
...
The notion of connection can be extended to entire graphs. An undirected graph $g$ is said to be *connected* if and only if $\univ e \in V^2 ( e \in \dom(\chi^+(g)))$.
