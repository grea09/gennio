...
The *subposition function* is the function $\triangledown$ that associates any two functions $f_1$ and $f_2$ to a new function $f_1 \sub f_2$ that associates anything associated by the first function $f_1$ but **not** by the second function $f_2$.
:::
...
<!--FIXME: Extra quantifiers and higher order logic + modal-->
...
In this chapter, we presented the tools we will use for the rest of the document along with its underlying formalism. These tools are comprised of a functionnal theory along with an algebra, first and higher order logic, sets, tuples, graphs, hypergraphs and sheaves. Those notions are mostly data structures and allows to express any models needed for our usage.
