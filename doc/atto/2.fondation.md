...
The notation $\none$ was chosen to represent the association arrow ($\to$) but with a dot instead of the tail of the arrow. This is meant to represent the fact that it inhibits associations.
...
::: example
From the previous example we can write $f_3 \sub f_2 = (1 \to 2)$. Since $f_2$ has the $2 \to 3$ associations in common with $f_3$ it is removed from the result.
...
$f_1 \vartriangle f_2 = f_2 \vartriangle f_1$   Superposition order doesn't affect the result.
...
<!--**Antoine**: Je n'utilise pas toujours le formalisme fonctionnel dans ces sections, car, cela rendrait les formules bien plus compliquées et n'apporte rien. C'est utilisé dans le 2.4.1 pour définir la base, puis j'utilise celle-ci pour définir le reste. Je dois d'abord finir les autres chapitres pour référencer là où c'est utilisé.-->
