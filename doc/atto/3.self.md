...
:::example
We can make a grammar for all sequencs of `A` using the rule `<scream> ::= "A"+`. If we want to make a rule that prevent the use of the letter `z` we can write `<no-sleep> ::= ~"z"`.
:::
...
::: example
With the input `!x = 0;` we parse `<SYMBOL> <ENTITY> <EQUAL> <LITERAL> <EOS>`. This cannot be a container since the new symbol is at the begining without any mirroring possible. It cannot be a parameter since no container is present. But this will conclude with the operator meta-rule as the new symbol precedes an entity. This becomes `<meta_entity> <EQUAL> <LITERAL> <EOS>` and becomes a valid statement.
:::

If all meta-rules fail, then the parsing fails and returns an error to the user like in classical occurences.
...
What are quantifiers? In SELF they differ from their mathematical counterparts. The quantifiers are special entities that are meant to be of a generic type that matches any entities including quantifiers. There are infinitely many quantifiers in SELF but they are all derived from a special one called the *solution quantifier*. We mentioned it briefly during the definition of the grammar $\bb{g}_0$. It is the language pendant of $\mu^\bullet$ and is used to extract and evaluate reified knowledge. 
...
In the field of logic there exists one special flavor of it called _modal logic_. It lays the emphasis upon the qualification of statements, and especially the way they are interpreted. This is a very appropriate example for SELF. The modality of a statement acts like a modifier, it specifies a property regarding its plausibility, origin or validity.

::: example
In the @fig:gossip, we present a case of three women gossiping, Alice, Becky and Carol. The presentation is inspired by the work of @schwarzentruber_hintikka_2018.
Here are a list of the statements in this example:
...
The following statement can be infered:
...
In the example, all modality are *emphasized*. One can notice an interesting property of these statements in that they are about other statements. This king of description is called *higher order knowledge*.

### Higher order knowledge

SELF is based on the ability to easily process higher order knowledge. In that case the term *order* refers to the order of abstraction of a statement. For such usages, an hypergraph structure such as SELF is using is a clear advantage in term of expressivity and ease of manipulation of those statements.

In the following listings, we present the example using RDF and SELF to encode the knowledge.
...
It is obvious that the SELF version is an order of magnitude more concise than RDF to express modal logic. In the RDF version we use the reified statements `:s1`, `:s2`, `:s3` and `:s4` along with a `:modality` anotation to express high order knowledge and modalities. In SELF, everything is infered by structure and one can start exploiting their database right away.
