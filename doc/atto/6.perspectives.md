...
![The latice formed by observations (top), matching plans, possible hypothesis and problem (bottom)](graphics/bouchard.svg)
...
This theory is based on the Belief, Desire and Intention (BDI) model. In our case the belief part is akin to the knowlwedge database, the desires are the set of possible goals (wheighted by cost) and the intent is the plan that achieve a selected goal.
...
**TODO : Schema of constraint encoding**
...
Now that the intuition is covered, we need to prove that the result of the planning process is indeed corelated to the probability of the agent pursuing that plan knowing the observations. We note the set of goals that can be pursued $G$ and the temporal sequence of observations $\cal{O}$. Using conditional probabilities, we seek to have a measure of $\bb{P}(g|\cal{O})$ for any goal $g$.
...
We assume that the observed agent is pursuing one of the knowns goals. The event of an agent pursuing a goal is noted $g$. This means that $\bb{P}(G) = \sum_{g\in G}\bb{P}(G) = 1$ because the event is certain.

Using conditional probabilities we can also note  $\bb{P}(G|\plan) = 1$ for a valid plan $\plan$ that achieves $g$.

From dirrect application of Bayes theorem and the previous assomptions, we have :

$$\bb{P}(\plan|\cal{O}) = \frac{\bb{P}(\cal{O}|\plan) \bb{P}(\plan)}{\bb{P}(\cal{O})} = \frac{\bb{P}(\cal{O}|\plan) \bb{P}(\plan|G) \bb{P}(G)}{\bb{P}(\cal{O})}$$ {#eq:plan-obs}

This means that we can write :

$$ \proba(G|\cal{O}) = \frac{\proba(\cal{O}|G)\poba(G)}{\proba(\cal{O})}$$ {#eq:goal-obs}

And from the total probability formula :

$$\proba(\cal{O}|G) = \sum_{\plan \in \plans_G} \proba(\cal{O}|\plan) \proba(\plan|G)$$ {#eq:obs-goal}
