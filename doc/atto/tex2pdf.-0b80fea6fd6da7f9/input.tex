...
%
\documentclass[11pt,a4paper,twoside,openright,titlepage,numbers=noenddot,headinclude,cleardoublepage=empty,openany]{scrreprt}
\usepackage{lmodern}
\usepackage{multicol}
...
\newcommand{\ifclkfamily}{\fontencoding{U}\fontfamily{ifclk}\selectfont}
\DeclareTextFontCommand{\textifclk}{\fontencoding{U}\fontfamily{ifclk}\selectfont}
...
Mathematics needs a fondation. Several theories have been proposed as
the fondation of mathematics (set theory, group theory, model theory,
etc). In this part, as an introduction to the fundamentals of maths and
logic, we propose another view of that fondation based on relations. The
...
\begin{axiom}[Symbolism]\label[axiom]{axi:symbolism}

Let be symbols.

\end{axiom}

\begin{axiom}[Relation]\label[axiom]{axi:relation}

Let be relations between symbols.

\end{axiom}

\begin{axiom}[Reflexivity]\label[axiom]{axi:reflexivity}

Relations are symbols.

\end{axiom}

While simple these axioms are very important. Indeed, it is not possible
to formally define yet because we are defining the formalism. This means
that we need to use relations and symbols to express their definition.
That isn't a big problem in itself since most of the human vocabulary,
for example, is circular. All languages have cyclical definitions for
basic terms.

The prefix notation delimited by parenthesis \(r(args)\) allows to get
the image of arguments associated by the relation \(r\). If a relation
only associate one symbol to one other symbol, we can also use the infix
notation e.g.~\(arg_1 r arg_2\).

\begin{definition}[Null]\label[definition]{def:null}

The \emph{null relation} is the relation between nothing and nothing. We
note it \(\gtrdot\).
...
The \emph{identity relation} is the relation that associates any symbol
to itself noted \(=\).

\end{definition}

An \textbf{expression} is any coherent combination of \textbf{terms}.
Each term must be either a terminal symbol or a relation properly
applied to arguments. This can be recursive as relations can take the
value of another relation as argument.

A relation can be referenced to by its symbol or defined anonymously
using the \emph{mapping relation} \(x \mapsto f(x)\) with \(f(x)\) being
a valid expression depending on \(x\).

\begin{definition}[Combination]\label[definition]{def:combination}

The \emph{combination relation} is the relation that associates any two
relations \(r_1\) and \(r_2\) to a new relation that associates any
symbols associated by \textbf{either} relations. Formally
\(r_1 \bowtie r_2 = x \mapsto y\) with \(r_1(x) = y\) \textbf{or}
\(r_2(x) = y\).
...
The \emph{superposition relation} is the relation that associates any
two relations \(r_1\) and \(r_2\) to a new relation that associates any
symbols associated by \textbf{both} relations. Formally
\(r_1 \vartriangle r_2 = x \mapsto y\) with \(r_1(x) = r_2(x) = y\).

\end{definition}

\begin{definition}[Complement]\label[definition]{def:complement}

The \emph{complement relation} is the relation that associates any
relations to a new relation that associates any symbols \textbf{not}
associated by the relation. Formally \(\triangledown r = x \mapsto y\)
with \(y \triangledown(=) x\).

We can also use this relation using the infix notation in the following
way : \(r_1 \triangledown r_2 = r_1 \vartriangle \triangledown(r_2)\) in
which case we call it \emph{subposition relation}.

\end{definition}

From now on we will also note \(\neq = \triangledown(=)\). We can also
note a few properties of these relations:
...
The \emph{composition relation} is the relation that associates any two
relations \(r_1\) and \(r_2\) to a new relation such that:
\(r_1 \circ r_2 = x \mapsto r_1(r_2(x))\).

\end{definition}

\begin{definition}[Inverse]\label[definition]{def:inverse}

The \emph{inverse relation} is the relation that associates any relation
to its inverse such that if \(y = r(x)\) then \(x = \bullet(r)(y)\).

We can also use an infix version of it with the composition of
relations: \(r_1 \bullet r_2 = r_1 \circ \bullet(r_2)\).
...
From now on we will use numbers and classical arithmetics on them. From
a fondation point of view, these are defined later using
\namecref{axi:peano} of \nameref{axi:peano}.
...
of the set.}'' \hfill George Cantor
(\protect\hyperlink{ref-cantor_beitrage_1895}{1895})
\end{quote}

For Cantor, a set is a collection of concepts and percepts. We define a
set using the notations in table \ref{tbl:set}.
...
\item
  ...
  \(\{n : n \in \bb{N} \land (n \le 4) \}\)
\end{itemize}

\end{definition}

The member relation is noted \(e \in \cal{S}\) to indicate that \(e\) is
an element of \(\cal{S}\). We note
...
\centering
\includegraphics{./tex2pdf.-0b80fea6fd6da7f9/799e1d4f806e7fef1a07698c8b63406a4de54109.pdf}
...
We can also define the set power recursively by \(\cal{S}^1 = \cal{S}\)
and \(\cal{S}^n = \cal{S} \times \cal{S}^{n-1}\).
...
The definition by intention uses \emph{set builder notation} to define a
...
\caption{\label{tbl:rel}List of classical symbols and syntax for
relational algebra.}
...
\(\langle e_1, e_2, e_n \rangle\) & \(n\)-uple also called a
relation. \\
\(\cal{S} \to \cal{S}\) & Association relation. Used for graph edges and
domain definition. \\
...
\begin{definition}[Relation]\label[definition]{def:rel}

A relation is effectively a subset of the cartesian product between
several sets:
\(r = \sigma \wp \left( \bigtimes_{i=1}^n \cal{S}_i \right)\) with
\(\sigma\) being the selection relation and \(n\) being the \emph{arity}
of the relation \(r\).

It can also be noted as a set of tuples each noted
\(\langle e_1, e_2, e_n \rangle\).

\end{definition}

We need to define some special relations often used for set
manipulation:

\begin{itemize}
\tightlist
\item
  The \textbf{substitution} that replace a variable in an expression
  \(\bb{e}\) such that: \((e \mapsto f(e))(\bb{e}(e)) = \bb{e}(f(e))\).
  The substitution is often used for function definition.
\item
  The \textbf{selection} that selects elements given a predicate \(?\)
  such that: \(\sigma_?(\cal{S}) = \{e : ?(e) \land e\in \cal{S}\}\).
  The choice selection \(\sigma_!(\cal{S})\) is a non deterministic
  choice of one element in \(\cal{S}\).
\item
  The \textbf{projection} that merges elements of a set given a filter
  \(f\) such that: \(\pi_f(\cal{S}) = \{ f(e) : e \in \cal{S}\}\). The
  default projection uses the identity relation \(=\) instead of \(f\).
\end{itemize}

Functions are a special case of relations that takes as value the
selected element of the last set. We note them
\(f: \left( \bigtimes_{i=1}^{n-1} \cal{S}_i \right) \to \cal{S}_n\). The
set \(\bigtimes_{i=1}^{n-1} \cal{S}_i\) is called the \emph{domain} of
the function \(\cal{D}(f)\) and the set \(\cal{S}_n\) is caled the
\emph{codomain} \(\cal{D}^{-1}(f)\). The number \(n-1\) is called the
degree of the function.

We can combine functions using the \emph{function composition operator}
\(g \circ f = x \mapsto g(f(x))\). The functional power of \(f\) noted
\(f^n\) is defined recursively as :

\begin{itemize}
\tightlist
\item
  \(f^0 = x \mapsto x\), \(f^1 = x \mapsto f(x)\) and
  \(f^{-1} = f(x) \mapsto x\)
\item
  \(f^n = f \circ f^{n+[n<0]-[n>0]}\) with \([?(n)]\) being an Iverson
  bracket (\([\top]=1\) and \([\bot]=0\)).
\end{itemize}
...
A graph is a mathematical structure \(g=(V,E)\) consisting of vertices
\(V\) (also called nodes) and edges \(E\) (arcs) that links two vertices
together. Each edge is basically a pair of vertices ordered or not
depending on if the graph is directed or not. We can write
\(E= \sigma\wp(V^2)\)
...
\centering
\includegraphics{./tex2pdf.-0b80fea6fd6da7f9/6d43c32872248ee643573f68f990c7ebc50bf2ee.pdf}
...
\centering
\includegraphics{./tex2pdf.-0b80fea6fd6da7f9/4c3eb5b426cd014dfc600af5fce10ec659344dd3.pdf}
...
\centering
\includegraphics{./tex2pdf.-0b80fea6fd6da7f9/192e36fc1f840aefd2b4f7c39f4c910cc062790c.pdf}
...
\subsection{Grammar and Parsing}\label{grammar-and-parsing}}
...
the heart of our ranking mechanism detailed in
...
to only choose the best provider (@alg:safeoperatorgraph at
...
The last part of this initialization (@line:populate) is the agenda
population that is detailed in the {populate} function. During this
step, we perform a search of alternatives based on the list of updated
fluents. Online updates can make the plan outdated relative to the
domain. This forms liar links :
...
this case the line \ref{line:emptygoal} of the algorithm \ref{alg:pop}
