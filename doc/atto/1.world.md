...
*(s p o) : ((s p o) subject s); *(s p o) : ((s p o) property p);      *(s p o) : ((s p o) object o); $\label{line:incidence}$
```
...
All statements up to @line:endquantifier are quantifiers definitions. On the left side we got the quantifier symbol used as a parameter to the solution quantifier using the operator notation. On the right we got the domain of the quantifier. The exclusive quan,tifier has as a range the empty set. For the existantial quantifier we have only a restriction of it not having an empty range. At last, the uniqueness quantifier got a set with only one element matching its variable.

#### Infering native properties

Each native property has its own structural definition:
...
* $\mu^-$ (at @line:first) is the solution quantifier discussed above.
