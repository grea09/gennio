...
**Symbol**                                                      **Description**
----------                                                      ---------------
$A, F, R, \mathbb{\Pi}, \mathbb{S}$                             Sets of actions, flaws, resolvers, plans and search space.
$\mathcal{l} \downdasharrow a$                                  Partial support of action $a$ by the causal link $\mathcal{l}$.
$\mathbb{\pi} \downarrow a$                                     Full support of action $a$ by plan $\mathbb{\pi}$
...
**FIXME** $F$ for fluents and flaws !!!!

In this section, a general formalism of automated planning is proposed. The goal is to explain what is planning and how it works. First we must express the knowledge domain formalism, then we describe how problems are represented and lastly how a general planning algorithm can be envisionned.

### Planning domain

The planning domain is rooted in the formalism of SELF. All elements are entities and all the previous relations are conserved.
...
Fluents represent world property that change over time. Each state of the world is simply a set of fluents.

::: {.definition name="State"} :::
A state is a set of fluent $s$. It is provided with a truth value like a fluent and can behave like one. The truth value is the conjunction of all fluents within the state $s \models \bigwedge_{f \in s} f$. A special *disjunction* relation can be used on fluents and states to create a and/or tree with the states being the and nodes, the disjuction expression being the or nodes and fluents being the leafs.
:::::::::::::::::::::::::::::::::::

**TODO**: Schema of the and/or tree.

**TODO**: AND/OR trees in chapter 1 + conjuction in domain as consistency

Another important part of the behavior of fluents is their ability to match and to being unified.

Matching is the relation $u^? : F^2 \times \mathcal{S} \to \mathcal{S}$ that affects any pair of fluents along with a context state to another state containing the context augmented with unification constraints or $\emptyset$ if the two fluents are contradicting one another given the context. If the two fluents are equal or have a different function, they do not influence the context. Opposite fluents will always contradict. The quantified or variable arguments may cause contradiction or add a constraint into the context.

The actual unification relation $u : F^2 \times \mathcal{S} \to F$ bind any matching fluents to another grounded fluent giving the constraints of the given context. 

**TODO**: Cartesian poduct in chap 1 + set F fluents

Both relations are generalized to states by merging the result of the following way: 

$$u(s_r, s_f) = \bigcup_{f_r \in s_f, f_f \in s_f} u(f_r, f_f, s_a)$$

with, $s_r$ being the reference state and $s_f$ being the formula state. The state $s_a$ is an accumulator that is result of the partial union of the previous matching iterations.
This formula is the same for $u^?$ and $u$. It is interesting to note that the relations doesn't need an additional parameters and share the same definition domain $\mathcal{S}^2 \to \mathcal{S}$ when taking states as arguments.
...
An action is a parametrized tuple $a(args)=\langle \mathit{pre}, \mathit{eff}, c, ¢, d, \mathcal{P}, \Pi \rangle$ where:

* $\mathit{pre}$ and $\mathit{eff}$ are states that are respectively the **preconditions and the effects** of the action.
* $c$ is the state representing the **constraints**.
* $¢$ is the intrisic **cost** of the action.
* $\delta$ is the intrinsic **duration** of the action.
* $\mathcal{P}$ is the prior **probability** of the action succeeding.
* $\Pi$ is a set of **methods** that decompose the action into smaller simpler ones.
::::::::::::::::::::::::::::::::::::::::::::::::::
...
Actions are often represented as state operators that can be applied in given state to alter it. The application of actions is done by using the actions as relations $a : \mathcal{S} \to \mathcal{S}$ defined as follow:

$$a(s) = 
\begin{cases}
  \emptyset,& \text{if } u^?(s, \mathit{pre}(a))=\emptyset\\
  u(s,\mathit{eff}(a)),& \text{otherwise}
\end{cases}$$
...
A domain $D$ is a set of **operators** which are fully lifted *actions*, along with all the relations and entities needed to describe their preconditions and effects.
:::::::::::::::::::::::::::::::::::

**FIXME**: Use $R$ as the set of relations ? What about entities ? $E_D$ ?
...
We note $l = a_s \xrightarrow{c} a_t$ the link between its source $a_s$ and its target $a_t$ caused by the set of fluents $s_c$. If $s_c = \emptyset$ then the link is used as an ordering constraint.
:::::::::::::::::::::::::::::::::
...
A planning algorithm, often called planner, is an exploration of a search space $\mathbb{S}$ partially ordered by an iterator $\phi^+_{\mathbb{S}}$ guided by a heuristic $h$. From any problem $p$ every planner can derive two informations imediately: 

* the starting point $\mathbb{s}_0 \in \mathbb{S}$ and
* the solution predicate $\models_p$ that gives the validity of any potential solution in the search space.

Formally the problem can be exprimed as a pathfinding problem in the dirrected graph $g_{\mathbb{S}}$ formed by the vertex set $\mathbb{S}$ and the adjacence function $\phi^+_{\mathbb{S}}$. The set of solutions is therefore expressed as:

$$\mathbb{S}^* = 
\left \{ \mathbb{s}^* : 
  \langle \mathbb{s}_0, \mathbb{s}^* \rangle \in \mathcal{E}_{\chi^*(g_{\mathbb{S}})} 
  \land \models_p(\mathbb{s}^*)  
\right \}$$
:::

In automated there are also other considerations about the search. 
...
Another aspect of planning lies in its timming. Indeed sometimes acting needs to be done before a dealine and planning is useful only durring a finite timeframe. In that aspect, several approaches exists:

* $\acidfree$ Optimal search without time limitation, finding the best solution everytime.
* $\Taschenuhr$ Anytime search, finding a solution and improving its quality until stopped.
* $\Interval$ Real-time search, being able to give a solution in a given time even if it is an approximation.

**TODO**: Chap 1 $\Interval$

#### General planner

A general planner $\mathbb{G}[\mathbb{S},\phi^+_{\mathbb{S}},h,\mathbb{t}](D,p)$ is an algorithm that can plan any formalism of automated planning. It takes two set of parameters:

* **Formalism dependant parameters**
  + $\mathbb{S}$ the search space
  + $\phi^+_{\mathbb{S}}$ the search iterator
  + $h$ the heuristic
  + $\mathbb{t}$ the problem transformation function
* **Domain depedant parameters**
  + $D$ the *planning domain*
  + $p$ the *planning problem*

The heuristic $h(\mathbb{s})$ gives off the shortest predicted distance to any point of the solution space. The exploration is guided by it by minimizing its value.

Other informations must be added to any problem $p$ in the form of constraints:

$$\left( \delta(\mathbb{G}) <= \delta^* \veeonwedge |\Pi(a_0)| = k \right) \in c(a_0)$$

The value for $k$ is extracted from problem it and the temporality is expressed using either $\land$, $\lor$ or $\land \bot \lor$ instead of $\veeonwedge$.

**TODO**: Chap 1 $\veeonwedge$

The transformation function $\mathbb{t}(p) = \langle \mathbb{s}_0, \models_p \rangle$ gives the starting point $\mathbb{s}_0$ and the solution predicate $\models_p$. This predicate is derived from the problem description and its contraints and contains the requirements for the diversity and temporality.

For the algorithm itself, we simply use a parameterized instance of the K* algorithm [@aljazzar_heuristic_2011, alg. 1]. This algorithm uses the classical algorithm A* to explore the graph while using Dijkstra on some sections to find the $k$ shortest paths. The parameters are as follow: $K^*(g_{\mathbb{S}}, \mathbb{s}_0, \models_p, h)$. The solution predicate contains the expression of the restriction of $k$ solutions, therefore, it superseeds the need for the $k$ parameter. We also add the heuristic $h$ to guide the A* part of the algorithm.

Of course this algorithm is merely an example of a general planner algorithm. Its efficiency hasn't been tested.

**TODO**: Schema of the full formalism
...
$$\mathbb{G}_{state} = \mathbb{G} \left[ \mathcal{S}, \bigcup_{a \in A} a(\mathbb{s}), h, \mathit{pre}(a_0), \mathit{eff}(a_0) \right]$$

For this formalism, we often set $k=1$ and $\delta^* = \infty$ as is customary in classical planning. It can also be specified as a backward search by inverting the application of $a$ with $a^{-1}$ and having the starting state as $\mathit{eff}(a_0)$ and solution predicate as $\mathit{pre}(a_0)$.
...
$$\mathbb{G}_{psp} = \mathbb{G} \left[ \Pi, \bigcup_{f \in \otimes(\mathbb{s})}^{r \in \odot^\pm_f} r(\mathbb{s}), h, \left(\{\mathbb{i}, \mathbb{g}\},\{\mathbb{i}\rightarrow \mathbb{g}\} \right), \otimes(\mathbb{s}) = \emptyset \right]$$

with $\mathbb{i}$ and $\mathbb{g}$ being the intial and goal steps of the plan $\mathbb{s}_0$ such that $\mathit{eff}(\mathbb{i}) = \mathit{pre}(a_0)$ and $\mathit{pre}(\mathbb{g}) = \mathit{eff}(a_0)$. The iterator is all the possible resolutions of all flaws on any plan $\mathbb{s}$ and the solution predicate is true when the plan has no more flaws.

Details about flaws, resolver and the overall Partial Order Causal Links (POCL) algorithm will be presented @later.

This approach usually can give partial plan if we set $\delta^* < \delta(\mathbb{s^*} \models p)$. This plan is not a solution but can eventually be engineered into having approximative properties relative to a solution.
...
$$\mathbb{G}_{cbp} = \mathbb{G} \left[ \mathcal{L}, repair(\mathbb{s}), h, query(\mathcal{L}, a_0), \mathbb{s} \Downarrow \mathit{eff}(a_0) \right ]$$

with $\mathcal{L}$ being the plan library. The planner selects efficiently a plan that fit the best with the intial and goal state of the problem. This plan is then repaired and validated iteratively. The problem with this approach is that it may be unable to find a valid plan or might need to populate and maintain a good plan library. For such case an auxiliary planner is used (preferably diverse with $k>1$).
...
$$\mathbb{G}_{prob} = \mathbb{G} \left[ \mathcal{S} \times A, \mathbb{s}_{+1} = \bigcup_{a \in A : a(s) \neq \emptyset}^{s = a_l(s_l) : pick(\mathbb{s}_{+1}) = \langle s_l, a_l \rangle} \langle s, a \rangle, h_{reward}, \bigcup_{a \in A : a(\mathit{pre}(a_0)) \neq \emptyset} \langle \mathit{pre}(a_0), a \rangle , s \models \mathit{eff}(a_0) \right]$$

The state $s$ is a state chosen from the frontiere. The frontiere is updated at each iteration with the application of a non-deterministically chosen pair of the last policy insertion. The search stops when all elements in the frontiere are goal states.
...
$$\mathbb{G}_{htn} = \mathbb{G} = \left [ \Pi, \bigodot _{a \in A_{\mathbb{s}} : \Pi_a \neq \emptyset}^{m = pick(\Pi_a)} \mathbb{s}, h, a_0, \forall a \in A_{\mathbb{s}} : \Pi(a) = \emptyset \right ]$$

**FIXME** Replace $\bigodot$ by a replace operator
