...
**Symbol**                    **Description**
----------                    ---------------
$A, F, R, \bb{\Pi}, \bb{S}$   Sets of actions, flaws, resolvers, plans and search space.
...
* $arg_{i \in [1,n]} \in \cal{E}$ are the arguments (possibly quantified).
...
This formula is the same for $u^?$ and $u$. It is interesting to note that the relations doesn't need an additional parameters and share the same definition domain $\cal{S}^2 \to \cal{S}$ when taking states as arguments.
...
An action is a parametrized tuple $a(args)=\langle \mathit{pre}, \mathit{eff}, c, ¢, d, \cal{P}, \Pi \rangle$ where:
...
* $\cal{P}$ is the prior **probability** of the action succeeding.
...
Actions are often represented as state operators that can be applied in given state to alter it. The application of actions is done by using the actions as relations $a : \cal{S} \to \cal{S}$ defined as follow:
...
A planning algorithm, often called planner, is an exploration of a search space $\bb{S}$ partially ordered by an iterator $\phi^+_{\bb{S}}$ guided by a heuristic $h$. From any problem $p$ every planner can derive two informations imediately: 

* the starting point $\bb{s}_0 \in \bb{S}$ and
* the solution predicate $\vdash_p$ that gives the validity of any potential solution in the search space.
...
$$\bb{S}^* = 
\left \{ \bb{s}^* : 
  \langle \bb{s}_0, \bb{s}^* \rangle \in \cal{E}_{\chi^+(g_{\bb{S}})} 
  \land \vdash_p(\bb{s}^*)  
\right \}$$
:::
...
A general planner $\bb{G}[\bb{S},\phi^+_{\bb{S}},h,\bb{t}](D,p)$ is an algorithm that can plan any formalism of automated planning. It takes two set of parameters:

* **Formalism dependant parameters**
  + $\bb{S}$ the search space
  + $\phi^+_{\bb{S}}$ the search iterator
  + $h$ the heuristic
  + $\bb{t}$ the problem transformation function
...
$$\left( \delta(\bb{G}) <= \delta^* \veeonwedge |\Pi(a_0)| = k \right) \in c(a_0)$$
...
The transformation function $\bb{t}(p) = \langle \bb{s}_0, \vdash_p \rangle$ gives the starting point $\bb{s}_0$ and the solution predicate $\vdash_p$. This predicate is derived from the problem description and its contraints and contains the requirements for the diversity and temporality.

For the algorithm itself, we simply use a parameterized instance of the K* algorithm [@aljazzar_heuristic_2011, alg. 1]. This algorithm uses the classical algorithm A* to explore the graph while using Dijkstra on some sections to find the $k$ shortest paths. The parameters are as follow: $K^*(g_{\bb{S}}, \bb{s}_0, \vdash_p, h)$. The solution predicate contains the expression of the restriction of $k$ solutions, therefore, it superseeds the need for the $k$ parameter. We also add the heuristic $h$ to guide the A* part of the algorithm.
...
$$\bb{G}_{state} = \bb{G} \left[ \cal{S}, \bigcup_{a \in A} a(\bb{s}), h, \mathit{pre}(a_0), \mathit{eff}(a_0) \right]$$
...
$$\bb{G}_{psp} = \bb{G} \left[ \Pi, \bigcup_{f \in \otimes(\bb{s})}^{r \in \odot^\pm_f} r(\bb{s}), h, \left(\{\bb{i}, \bb{g}\},\{\bb{i}\rightarrow \bb{g}\} \right), \otimes(\bb{s}) = \emptyset \right]$$

with $\bb{i}$ and $\bb{g}$ being the intial and goal steps of the plan $\bb{s}_0$ such that $\mathit{eff}(\bb{i}) = \mathit{pre}(a_0)$ and $\mathit{pre}(\bb{g}) = \mathit{eff}(a_0)$. The iterator is all the possible resolutions of all flaws on any plan $\bb{s}$ and the solution predicate is true when the plan has no more flaws.

Details about flaws, resolver and the overall Partial Order Causal Links (POCL) algorithm will be presented @later.

This approach usually can give partial plan if we set $\delta^* < \delta(\bb{s^*} \vdash p)$. This plan is not a solution but can eventually be engineered into having approximative properties relative to a solution.
...
$$\bb{G}_{cbp} = \bb{G} \left[ \cal{L}, repair(\bb{s}), h, query(\cal{L}, a_0), \bb{s} \Downarrow \mathit{eff}(a_0) \right ]$$

with $\cal{L}$ being the plan library. The planner selects efficiently a plan that fit the best with the intial and goal state of the problem. This plan is then repaired and validated iteratively. The problem with this approach is that it may be unable to find a valid plan or might need to populate and maintain a good plan library. For such case an auxiliary planner is used (preferably diverse with $k>1$).
...
$$\bb{G}_{prob} = \bb{G} \left[ \cal{S} \times A, \bb{s}_{+1} = \bigcup_{a \in A : a(s) \neq \emptyset}^{s = a_l(s_l) : pick(\bb{s}_{+1}) = \langle s_l, a_l \rangle} \langle s, a \rangle, h_{reward}, \bigcup_{a \in A : a(\mathit{pre}(a_0)) \neq \emptyset} \langle \mathit{pre}(a_0), a \rangle , s \vdash \mathit{eff}(a_0) \right]$$
...
$$\bb{G}_{htn} = \bb{G} = \left [ \Pi, \bigodot _{a \in A_{\bb{s}} : \Pi_a \neq \emptyset}^{m = pick(\Pi_a)} \bb{s}, h, a_0, \forall a \in A_{\bb{s}} : \Pi(a) = \emptyset \right ]$$
