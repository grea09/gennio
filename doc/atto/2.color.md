...
Another language used by the 7^th^ IPC's uncertainty track is RDDL [@sanner_relational_2010a]. This language has been choosen because of its ability to express problems that are hard to encode in PDDL or PPDDL. Indeed, RDDL is capable of expressing POMDP and DBN in planning domains. This along with complex probability laws allows for easy implementation of most probabilistic planning problems. Its syntax differs greatly from PDDL, and seems closer to scala or C++. An example is provided in @lst:rddl from @sanner_relational_2010a.


``` {#lst:rddl .cpp caption="Example of RDDL syntax by Sanner. (slightly modified for compatibility purpose)" escapechar=$}
////////////////////////////////////////////////////////////////////////
...
domain prop_dbn {
	...
	cpfs {
		...
		p_ = if (p ^ r) then Bernoulli(.9) else Bernoulli(.3);
						
		q_ = if (q ^ r) then Bernoulli(.9) 
						else if (a) then Bernoulli(.3) else Bernoulli(.8);

		// KronDelta is like a DiracDelta, but for discrete data (boolean or int)
		r_ = if (~q) then KronDelta(r) else KronDelta(r <=> q);										
	};
