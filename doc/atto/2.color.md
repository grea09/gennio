...
A more recent example of HTN formalism comes from the PANDA framework by @bercher_hybrid_2014. This framework is considered the current standard of HTN planning and allows for great flexibility in domain description. 
...
* URI changed into simpllle names.
* type-fun added (for no reasons ?)
* axioms → facts, vars → freevars, implies → ←
* actions parameters and values merged into a simpler notation.
