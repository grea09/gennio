...
This formula is the same for $u^?$ and $u$. It is interesting to note that the relations doesn't need an additional parameters and share the same definition domain $\mathcal{S}^2 \ to \mathcal{S}$ when taking states as arguments.
...
Probabilistic planning tries to deal with uncertainty by generating a policy instead of a plan. The initial problem holds probability laws that govern the execution of any actions. It is often accompagnated with a reward function instead of a deterministic goal.

$$\mathbb{G}_{prob} = \mathbb{G} \left[ \mathcal{S} \times A, \bigcup_{a \in A : a(s) \neq \emptyset}^{s = a_l(s_l) : pick(last(\mathbb{s})) = \langle s_l, a_l \rangle} \langle s, a \rangle, h_{reward}, \bigcup_{a \in A : a(\mathit{pre}(a_0)) \neq \emptyset} \langle \mathit{pre}(a_0), a \rangle , s \models \mathit{eff}(a_0) \right]$$
