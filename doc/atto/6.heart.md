...
![Refinement process of HEART](graphics/refinement.svg){#fig:refinement}
...
In such a case, the planning model can be subject to uncertainty. Indeed, the execution of an action can fail because an external event changed a precondition required to do it or because the model itself is be inaccurate.
...
Another issue is caused by the need to get any existing partial plan as an input. This plan can contain any problems and inconsistency since there are trivial ways to ensure consistency between the plan given and the new initial state or goal.k

### Operator Graph

In order to make POCL faster we experimented a mean to build a dependency graph for operators in the domain, computed at domain compilation time. The idea is to extract a partial plan from this graph by pruning it based on the provided goal and use that plan to quick start the planning process.
...
### Usefulness Heuristic {#sec:usefull}
