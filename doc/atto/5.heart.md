...
The second flaw in the @fig:flaws is the threat between $a_2$ and a causal link outgoing from $a_3$. This happens because nothing prevents $a_2$ to be executed after $a_3$ and undo the fluent $f_2$ needed by the next step.
:::
...
That first planner is a prototype destined to test the feasibility of a plan repairing approach using PSP. This is done through the addition of new flaw types in a classical PSP algorithm.
