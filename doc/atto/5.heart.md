...
* **Subgoals**, also called *open condition* that are yet to be supported by a *provider* $a_p$. We note subgoals $\otimes^\downarrowbarred_{\bb{\pi}} = \langle a_n, f \rangle$.
* **Threats** are caused by steps that can break a causal link with their effects. They are called *breakers* of the threatened link. A step $a_b$ threatens a causal link $l_t = a_p \xrightarrow{f} a_n$ if and only if $(\neg f \land \eff(a_b)) \land (a_b \not\succ a_p \land a_n \not\succ a_b)$. Said otherwise, the breaker can potentially cancel an effect of a providing step $a_p$, before it gets used by its needer $a_n$. We note threats $\otimes^\dagger_{\bb{\pi}} = \langle a_n, f, a_b \rangle$.
:::
