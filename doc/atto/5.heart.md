...
\Function{bind}{Action $a$}
    \ForAll{$f \in \pre(a)$}
        \If{$f \in providing$}
            \ForAll{$\plan \in$ \Call{get}{$providing$, $f$}}
                \State Link $l \gets$ \Call{getEdge}{$\plan$, $a$} \Comment{Create the link if needed}
                \State $l \gets l \cup \{f\}$ \Comment{Add the fluent as a cause}
            \EndFor
        \EndIf
    \EndFor
