...
Let's the identity function be <+eq> that associates every function to itself. \mathnote{$(\eq)  = x \asso x$} This function is therefore transparent as by definition $=(x)$ is the same as $x$. It can be described by using it as **affectation** or aliases to make some expressions shorter or to define any function.
:::
...
^[Using [@def:currying; @def:application], we can note]
Let's the term <+asso> be the function that associates two expressions to another function that associates those expressions. This special function is derived from the notation of *morphisms* of the category theory. \mathnote{$(\asso) = x \asso (f(x) \asso f)$}
:::
...
\mathnote{$\llp f \rrp = (x \asso \llp f(x)\rrp)$}
...
\mathnote{$\llbp f\rrbp = \llbp x, y \asso f(x)(y) \rrbp^+$}
...
\mathnote{$y = f(x)$} We note the application of $f$ with an argument $x$ as $f(x)$. The application allows to recover the image $y$ of $x$ which is the value that $f$ associates with $x$.
:::
...
::: example
Applying this to basic arithmetic for illustration, it is possible to create a function that will triple its argument by making a partial application of the multiplication function $×(3)$ \mathnote{$×(3)=x \asso 3×x$} so we can write the operation to triple the number $2$ as $×(3)(2)$ or $×(2,3)$ or with the infix notation $2×3$.
:::

::: {.definition #def:null name="Null"}
The *null function* is the function between nothing and nothing. We note this function <+none> . \mathnote{$\none = \none \asso \none$}
:::
...
\mathnote{$\super = (\comb) \comb (f,f \asso f)$}
...
\mathnote{$\sub = f_1, f_2 \asso f_1 \comb (f_1 \super f_2)$}
...
A function noted $q$ that has as only values either <+tru> or <+fls> is called a **predicate**.\mathnote{$\dom(\inv q) = \{\fls, \tru\}$}
...
* The *universal quantifier* <+univ> meaning _"for all"_.\mathnote{$\univ = \sol(\land)$}
* The *existential quantifier* <+exist> meaning _"it exists"_.\mathnote{$\exist = \sol(\lor)$}
...
* The *uniqueness quantifier* <+uniq> meaning _"it exists a unique"_.\mathnote{$\uniq = \sol(=(1) \comp +)$}
* The *exclusive quantifier* <+exclu> meaning _"it doesn't exist"_.\mathnote{$\exclu = \sol(\lnot \comp \land)$}
...
* The *solution quantifier* <+sol> meaning _"those"_.\mathnote{$\sol = f, x, q \asso \lBrace f(x) \such q \rBrace$}
...
Using our functional foundation, we can define any set as a predicate $\cal{S} = e \asso \tru$ with $e$ being a member of $\cal{S}$. This allows us to define the member function noted $e \in \cal{S}$ to indicate that $e$ is an element of $\cal{S}$.\mathnote{$\in = e, \cal{S} \asso \cal{S}(e)$}
...
The *function of specification* (noted <+such>) is a function that restricts the validity of an expression given a predicate.\mathnote{$\such = f, q \asso f \sub (\dom(q = \fls) \to \dom(\inv f))$} It can intuitively be read as _"such that"_.
:::
...
* *Adjacency*: $\con_{\adja} = v \asso \{ e: v \in \con_{\inci}(e) \}$\mathnote{Also: $\con_{\adja} = \inv \con_{\inci}$}
...
Usually, graphs are noted $g=(V,E)$ with the set of vertices $V$ (also called nodes) and edges $E$ (arcs) that links two vertices together. Each edge is classically a pair of vertices ordered or not depending on whether the graph is directed or not.\mathnote{$E \subseteq V^2$}
