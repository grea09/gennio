...
PDDL+ is an extension of <+pddl> 2.1 that handles processes and events [@fox_pddl_2002]. It can be viewed as similar to <+pddl> 3.1 continuous effects but it differs on the expressivity. A process can have an effect on fluents at any time. The effect can happen either from the agent's own doing or being purely environmental. It might be possible in certain cases to model this using the durative actions, continuous effects and timed initial literals of <+pddl> 3.1.
