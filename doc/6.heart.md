...
First we will discuss existing planning algorithms of the sort. Next we propose and evaluate our first planner that uses plan repair instead of replanning. And finally, we present a hierarchical planner that is able to produce intermediary abstract plans at any time of its resolution.
...
![Venn diagram of the expressivity classes of <-htn> paradigms.](graphics/htn-expressivity.svg)
...
<!-- TODO Explain schema and table and cite them.-->
...
Another issue is caused by the need to get any existing partial plan as an input. This plan can contain any problems and inconsistency since there are trivial ways to ensure consistency between the plan given and the new initial state or goal.
